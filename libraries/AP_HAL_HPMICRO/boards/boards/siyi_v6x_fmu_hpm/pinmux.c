...
void init_uart_pins(UART_Type *ptr)
{
    if (ptr == HPM_UART0) {
        ...
        HPM_IOC->PAD[IOC_PAD_PE30].FUNC_CTL = IOC_PE30_FUNC_CTL_UART7_RXD;
        HPM_IOC->PAD[IOC_PAD_PE31].FUNC_CTL = IOC_PE31_FUNC_CTL_UART7_TXD;
    }
}
...
void init_sdxc_cd_pin(SDXC_Type  *ptr, bool as_gpio)
{
    ...
    if (ptr == HPM_SDXC0) {
        if (as_gpio) {
            /* SDXC1.CDN */
            uint32_t cd_func_alt = IOC_PE18_FUNC_CTL_GPIO_E_18;
            HPM_IOC->PAD[IOC_PAD_PE18].FUNC_CTL = cd_func_alt;
            HPM_IOC->PAD[IOC_PAD_PE18].PAD_CTL = cd_pad_ctl;
        }
    }
}
...
void init_pwm_pins(PWM_Type *ptr)
{
    if (ptr == HPM_PWM3) {
        ...
        HPM_IOC->PAD[IOC_PAD_PA19].FUNC_CTL = IOC_PA19_FUNC_CTL_PWM1_P_7;

        HPM_IOC->PAD[IOC_PAD_PA20].FUNC_CTL = IOC_PA20_FUNC_CTL_PWM1_P_6;

        HPM_IOC->PAD[IOC_PAD_PA24].FUNC_CTL = IOC_PA24_FUNC_CTL_PWM1_P_5;

        HPM_IOC->PAD[IOC_PAD_PA25].FUNC_CTL = IOC_PA25_FUNC_CTL_PWM1_P_4;
    }
}
